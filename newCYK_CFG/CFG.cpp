...
void CFG::setTerminals(const vector<terminal>& _terminals) {
	terminals = _terminals;
}

void CFG::setVariables(const vector<variable>& _variables) {
...
vector<terminal> CFG::getTerminals() const{
	return terminals;
}
...
vector<terminal> CFG::parseTerminals(string parsedTerminals){

	vector<terminal> foundTerminals;
...
vector<variable> CFG::parseVariables(string parsedVariables){

	vector<variable> foundVariables;
...
terminal* CFG::findTerminal(string name){
	for(unsigned int i = 0; i < terminals.size(); i++){
		if(terminals[i].getContent() == name) return &terminals[i];
	}
	return NULL;
}
