...
void CNF::removeEpsylon(){
	...
	//now we know all variables with an epsylon transition
	...
	for (auto variable : oldCFG.variables){
		for (auto production : oldCFG.getProductions(variable)){
			...
			for (auto string : newProd){
				...
				if ( rule.second.size()>0 && !newCFG.isInMultimap(rule) ){//if production doesn't already exist and it isn't the empty string, add it to the productions
...
stringVec CNF::createEpsylonProd(charVec epsylonVar, std::string prod){
